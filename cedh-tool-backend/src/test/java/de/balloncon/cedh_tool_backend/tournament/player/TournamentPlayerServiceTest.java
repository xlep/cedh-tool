...
class TournamentPlayerServiceTest {
  ...
  void testDrawScenario() {
    tournamentPlayerService.calculateAndAssignNewScores(
        tournament.getId(), new HashMap<>(seatMap), null);
    ...
    Map<Integer, BigDecimal> expectedScores =
        Map.of(
    ...
    for (TournamentPlayer tournamentPlayer : updatedPlayers) {
      ...
      assertThat(tournamentPlayer.getScore())
          .withFailMessage(
              "Seat %d expected score %s but got %s", seat, expected, tournamentPlayer.getScore())
