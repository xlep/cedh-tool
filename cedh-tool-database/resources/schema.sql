...
(
  id   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name VARCHAR,
  mode TEXT CHECK (mode IN ('SWISS', 'HARERUYA'))
);
...
WITH ids(id) AS (SELECT UNNEST(ARRAY[
     ...
     indexed_ids AS (SELECT id, row_number() OVER () AS rn
                     FROM ids),
     names AS (SELECT gs.i,
               ...
               FROM generate_series(1, 60) AS gs(i),
                    LATERAL (
                             SELECT ARRAY['James',
                                    ...
                                    'Sarah',
                                    'Karen']
  AS firstnames, ARRAY['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez', 'Hernandez', 'Lopez', 'Gonzalez', 'Wilson', 'Anderson', 'Thomas', 'Taylor', 'Moore', 'Jackson', 'Martin']
  AS lastnames
  ) AS names
  )
INSERT
INTO player (id, nickname, firstname, lastname)
...
VALUES ('e29fbe3f-1755-43cc-a27a-393ec6d80a09', '22321b1c-cade-4e9e-866c-34610510922b', 1000.000),
